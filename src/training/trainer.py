...
class MAETrainer(BaseTrainer):
    def __init__(self, cfg, *args, **kwargs):
        ...
        self.mean_type_loss = MeanMetric(compute_with_cache=False).to(self.device)

    def train_step(self, batch):
        inputs, recon, padding_mask, mask = self.model(batch)

        loss, stats = self.criterion(inputs, recon, padding_mask, mask)
        ...
        self.mean_type_loss.update(stats['type_loss'].detach())
        self.mean_value_loss.update(stats['val_loss'].detach())
    ...
    def train_epoch(self, *args, **kwargs):
        ...
        out.update(dict(
            ...
            type_loss=self.mean_type_loss.compute(),
        ))

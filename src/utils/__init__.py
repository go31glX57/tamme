...
def pad_sequence(x, *args, **kwargs):
    ...
    :return the output of torch.nn.utils.rnn.pad_sequence or None if all values in x are None or x is empty
    """
    n_features = None
    ...
    for i, el in enumerate(x):
        if el is not None and el.numel() > 0:
            if el.ndim < 2:
                ...
                n_features = el.shape[-1]

    if n_features is None:  # i.e. not a single element present in the whole batch
        return None
    ...
    for el in x:
        if el is not None and el.numel() > 0:
            if needs_unsqueeze:
                filled.append(el.unsqueeze(-1))
            else:
                filled.append(el)
        else:
            filled.append(torch.empty((0, n_features)))

    out = rnn_pad_sequence(filled, *args, **kwargs)
